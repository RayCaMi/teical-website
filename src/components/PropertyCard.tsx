...
export function PropertyCard({ data }: { data: PropertyData }) {
  return (
    <Link to={`/imovel/${data.id}`} className="bg-white/80 rounded-xl overflow-hidden border border-transparent hover:border-secondary transition-all group flex h-40 w-full min-w-[260px] lg:min-w-[320px] relative">
      ...
      <div className="p-4 flex flex-col justify-between w-full min-w-0">
        <div className="flex flex-col gap-1">
          <h3 className="text-primary font-bold text-sm truncate pr-6">{data.nome}</h3>
          <p className="text-gray-600 text-[10px] truncate">{data.cidade}, {data.uf}</p>
        </div>
